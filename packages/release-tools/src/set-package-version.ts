...
const bumpWorkspaceDeps = ({ packageJson, version, scope }: { packageJson: Record<string, unknown>, version: string, scope: string }) => {
...
interface Args { packagePath: string, version: string, scope?: string }
 ...
 **/
export const setPackageVersion = async ({ packagePath, version, scope }: Args) => {
