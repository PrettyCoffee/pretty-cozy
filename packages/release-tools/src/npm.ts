...
interface LoginArgs { scope?: string }
/** Log in to npm with the specified scope.
 * @param args.scope The npm scope to use.
 */
const login = async (args: LoginArgs) => exec(`npm login ${stringifyArgs(args as Record<string, unknown>)}`)

interface InstallArgs { force?: boolean, dryRun?: boolean }
...
interface PublishArgs { dryRun?: boolean, workspace?: string | string[], workspaces?: boolean, access?: "restricted" | "public" }
 ...
 */
const publish = (args: PublishArgs) => exec(`npm publish ${stringifyArgs(args as Record<string, unknown>)}`)
