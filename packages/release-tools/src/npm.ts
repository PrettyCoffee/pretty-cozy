 ...
 *
 * @returns A promise that resolves to the script's text output or null if the
 *   script failed.
 */
const run = async (script: string) => {
  try {
    return await $`npm run ${script}`.text()
  } catch {
    return null
  }
}
...
const install = async (args: InstallArgs) => {
  await $`npm i ${stringifyArgs(args as Record<string, unknown>)}`.quiet()
}
