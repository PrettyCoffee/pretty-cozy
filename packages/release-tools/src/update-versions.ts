...
interface Args { root: PackageInfo, workspaces: PackageInfo[], version: string, scope: string }
export const updateVersions = async ({ root, workspaces, version, scope }: Args) => {
  await Promise.all([
